...
const Cart = ({cartProducts, onAddToCart, onRemoveFromCart, checkout}) => {
    ...
    const itemsPrice = cartProducts.reduce((counter,product) => counter + product.price * product.amount, 0);
    const tax = itemsPrice * 0.20
    ...
    return (
        <div className='cart-area block'>
            ...
            {cartProducts && cartProducts.map((item) => (
                <Row key={item.id}>
                    <Col className='col-4 text-break'> item{item.id} </Col> {/*{product.name}*/}
                    <Col className='col-4'>
            ...
            {cartProducts.length !== 0 && (
                <>
                    ...
                    <Row>
                        ...
                        <Col className='col-5 text-end'> <strong> ${totalPrice} </strong> </Col>
                    </Row>
                    <hr/>
                    <Row>
                        <Col>
                            <button id='cart-checkout' onClick={checkout}> Checkout </button>
                        </Col>
                    </Row>
                </>
            )}
